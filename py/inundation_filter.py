...
def inundation(ins,outs):
    ...
    flood_triangles_gdf.to_file('shp/below_triangles_0.03.shp')
    ...
    below_polygon_gdf = gpd.GeoDataFrame(crs=crs,geometry=[i.buffer(5, join_style=1).buffer(-5.0, join_style=1) for i in below_polygon if not i.is_empty])
